...
class TDMPC2(torch.nn.Module):
	...
	def update_pi(self, zs, task):
		...
		if self.cfg.use_caps: # compute TS loss
			act, _ = self.model.pi(zs[:-1], task)[0]
			next_act, _ = self.model.pi(zs[1:], task)[0]
	...
	def _update(self, obs, action, reward, terminated, task=None):
		...
		with torch.no_grad():
			...
			next_z = z_buf[1:] # get the latent state of the next observations in the horizon
		...
		decoder_loss = decoder_loss / (self.cfg.horizon + 1)
